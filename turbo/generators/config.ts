...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
  plop.setGenerator('init', {
    description: 'Generate a new package for the xystack Monorepo',
    prompts: [
      {
        type: 'input',
        name: 'name',
        message: 'What is the name of the package? (You can skip the `@xystack/` prefix)',
      },
      {
        type: 'input',
        name: 'deps',
    ...
    actions: [
      (answers) => {
        if ('name' in answers && typeof answers.name === 'string' && answers.name.startsWith('@xystack/')) {
          answers.name = answers.name.replace('@xystack/', '')
        }
