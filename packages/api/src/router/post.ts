...
post.get('/:id', zValidator('param', z.object({ id: z.number() })), async (c) => {
...
post.post('/', zValidator('json', createPostSchema), async (c) => {
...
post.delete('/:id', zValidator('param', z.object({ id: z.number() })), async (c) => {
  const id = c.req.valid('param').id
  const db = c.get('db')
